        ...
        {items.map((stat) => (
          <div key={stat.id} className="bg-white rounded-xl shadow p-6 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <input type="number" value={stat.value} onChange={(e) => updateStat(stat.id, 'value', e.target.value)} className="border rounded p-2 text-sm w-full sm:w-24" />
              ...
              <button onClick={() => saveStat(stat)} className="bg-primary text-white px-4 py-2 rounded text-sm font-semibold whitespace-nowrap">Enregistrer</button>
            </div>
          </div>
        ))}
      </div>
